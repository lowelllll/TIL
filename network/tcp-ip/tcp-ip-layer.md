...
### MAC 주소를 모를때?
수신인의 IP 주소는 알고 MAC 주소는 모를 때는 ARP를 사용한다.

#### 브로드캐스트 MAC 주소
> 같은 데이터 링크 내에 있는 모든 기기에게 보낼 수 있는 MAC 주소  

- 주소는 모든 비트에 1이 들어간 'ff:ff:ff:ff:ff:ff'
- 브로드캐스트 MAC 주소는 라우터를 넘어갈 수 없음.

#### 수신인의 MAC 주소 조사
데이터 링크 내에선 MAC 주소를 사용하여 기기를 특정하기 때문에 IP 주소만으로는 수신인에게 전달 할 수없음.  

### ARP 프로토콜
> IP 주소를 가지고 상대 MAC 주소를 조사할 때 사용하는 프로토콜.    

1. MAC 주소를 알고싶은 컴퓨터의 IP 주소를 ARP 패킷에 적고 브로드캐스트 MAC 주소 앞으로 보냄.
    - ARP 패킷은 데이터 링크층의 프로토콜에서 캡슐화 되어 송신됨.
2. 수신인 중 자신의 IP 주소면 MAC 주솔르 적은 ARP 패킷을 보냄. 아니면 파기함.

## 네트워크 연결 방법
- 버스형
    - 축이 되는 케이블 선에서 선을 빼서 그 끝에 노드(연결된 기기)를 연결.
- 링형
    - 이웃하는 양쪽 노드를 연결하여 링 모양으로 연결.
    - 링 내에서 노드의 고장이나 케이블 절단 등이 한 군데라도 일어나면 전체가 통신되지 않음.
- 스타형
    - 하나의 노드(일반적으로 허브)를 사이에 두고 다른 노드를 연결.
    - 네트워크를 집중 관리, 중심이 되는 노드가 고장나면 전체에 영향줌. 중심이 되는 노드가 네트워크를 총괄함.
- 망형
    - 모든 노드를 1:1로 연결.
    - 일부 케이블이나 노드가 고장나도 그곳을 피해 통신을 할 수 있음 (WAN의 접속 형태)

## 이더넷
> 가장 많이 보급되어있는 데이터 링크

### 이더넷 구조
네트워크상의 모든 기기에게 신호를 보내고 해당하는 기기만 받는 방법으로 프레임을 주고 받음.  
사용하는 케이블이나 통신 속도에 따라 몇가지의 종류가 있음.

<b>노드 A에서 노드 B로 데이터를 송신하는 과정</b>
1. 송신하기 전 송신 중인 다른 노드가 없는지 확인한다.
2. MAC 주소 앞으로 데이터를 보냄.
3. 자기 앞으로 온 것은 회수하고 아니면 파기함.
4. 충돌을 감지 했을 때는 조금 있다가 다시 보냄. (동시에 프레임을 보내면 데이터가 충돌해 깨지는 경우가 있음)

이러한 주고 받기 방식을 <b>CSMA/CD</b> 방식이라고 함.

## 토큰 링
> 프레임이 충돌하지 않도록 권리를 얻은 컴퓨터만 송신할 수 있는 데이터 링크

### 토큰 링 구조
네트워크 상을 흐르고있는 <b>토큰</b>이라는 프레임을 사용하여 통신함.  
토큰을 취득한 컴퓨터만 송신할 수 있으므로 프레임 충돌이 발생하지 않음. 

#### 토큰 패싱 (token passing) 방식
1. 송신할 때 네트워크 상을 흐르고 있는 토큰을 회수함.
2. 회수한 토큰에 프로토콜을 추가해서 송신함.
3. 각 노드는 보내진 패킷을 일단 받고 자기 앞으로 온 것이 아니면 다시 보냄.
4. 송신된 프레임은 수신인에게 전달될 때 까지 네트워크 안을 순서대로 흐름. 
5. MAC 주소를 보고 자기 앞으로 온 것 이면 프레임을 복사함.
6. 수신측은 받았다는 것을 나타내는 표시(수령 신호)를 트레일러에 적어 다시 보냄.
7. 송신측이 수령 신호가 붙은 패킷을 받으면 토큰만 송신하고 종료함.

## 기타 데이터 링크
### FDDI (Fiber Distributed Data Interface)
> 광섬유 케이블을 사용한 토큰 패싱 방식의 데이터 링크.

이중 링을 만들고 보통 사용하는 링(1차 링)이 끊어져도 긴급 사용 링(2차 링)으로 통신을 계속 할 수 있음.

### 무선 랜
> 전파나 적외선을 사용하여 통신.

통신 방법 (<b>CSMA/CA</b> 방식)
1. 수신인 지정에는 MAC 주소를 사용하여 송신.
2. 무선 기지국에서 디지털파를 전파로 변환하고 데이터 링크 내의 모든 기기에 발신.
3. 기지국에서 온 전파를 수신하고 자기 앞으로 온 것일 때는 회수, 아니면 파기함.

### 블루투스
> 2.4GHz대의 전파를 사용하여 통신하기 위한 규격.  

휴대전화,PDA 같은 작은 소형에 전력 용량이 적은 컴퓨터 등에 이용함.
...
- 사용자 인증기능이 추가된 프로토콜.
- 두 대의 기기 간에 통신을 확립 한 후에 주고 받기를 시작함.

### 통신을 확립하는 절차
1. 통신을 하지 않을 때 상대에게 통신 요청을 보냄.
2. 접속할 자격을 가지고 있는 사용자인지 아닌지 확인.(사용자 인증)
3. 요청인이 통신 조건에 대해 상담.
4. 통신을 시작.

<b>PPP는 1:1 통신이므로 MAC 주소는 사용하지 않음.</b>

### 전화선 접속
- PPP는 주로 전화선을 사용하여 인터넷을 접속할 때 사용함.  
- 아날로그 회선이 아닌 디지털 회선 ISDN을 사용할 수도 있음.  

<b>엑세스 서버</b>  
전화선이나 ISDN 회선을 사용하여 인터넷에 접속하는 경우 필요함.(사용자 인증을 하기 위해서)

## PPPoE
> 이더넷 상에 있는 두 대의 컴퓨터 간에 인증을 수행할 수 있도록 한 것. 

xDSL, CATV 회선을 사용하여 엑세스 서버를 경유하여 인터넷을 접속할 때 사용함.

1. PPPoE로 통신상대와 연결함.
2. PPP를 상용하고 사용자 인증 등을 수행.
3. 통신을 시작

### xDSL
> ADSL,VDSL,SDSL 등 전화선을 이용한 디지털 통신 기술을 총칭한 말

- VDSL,SDSL : 업로드와 다운로드의 통신속도가 같음.
- ADSL : 업로드와 다운로드의 통신속도가 다름.(다운로드가 빠름)

## 데이터 링크층 및 물리층에서 통신을 보조하는 기기

- 리피터
    > 네트워크 상에 신호를 보정하는 기기, 신호의 일그러짐을 방지하는 기기.
    - 통신 매체나 주위 환경, 통신 거리등의 영향을 받아 신호가 약해지는 경우, 약해지는 정도가 심해지면 0인지 1인지 판단을 하지 못함. 그걸 보조해줌 
- 브리지
    > 신호의 보정을 덧 붙여 서로 다른 두 개의 데이터 링크를 연결하는 기능을 가진 기능.(데이터 링크층 및 물리층의 통신을 보조)
    - 수신처의 MAC 주소를 보고 다른 데이터 링크 앞으로 온 것이면 보내고 같은 데이터 링크 앞으로 온 것이면 파기함.
- 허브
    > 네트워크 상에서 케이블을 분기하기 위한 기기
    - 하나의 신호를 여러 개의 케이블로 보내기 위해 신호를 증폭할 필요가 있다는 점에서 리피터의 역할도 갖고 있음.
    - 받은 신호를 보정 , 증폭 -> 모든 접속구로 보냄 -> 수신처를 판단하지 않고 접속되어 있는 모든 컴퓨터로 보냄. (송신측 제외)
- 패시브 허브
    > 신호의 보정, 증폭 기능을 가지고 있지 않은 허브
- 스위칭 허브
    > 수신처의 MAC 주소를 보고 여러 개의 접속처 중에서 특정 노드엠나 신호를 보내는 기능을 갖고 있는 허브.
    - 수신처를 확인하고 갈 곳을 배당하기 때문에 일반 허브보다 통과에 시간이 걸림.
